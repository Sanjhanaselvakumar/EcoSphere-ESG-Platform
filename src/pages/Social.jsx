...
import { Plus, Download, Users, Heart, BookOpen, TrendingUp, RefreshCw } from 'lucide-react'
...
import { fetchCsrActivities } from '@/services/api'

import { csrActivities as staticCSR, diversityMetrics, trainingData, engagementData } from '@/data/socialData'
...
function normaliseCSR(items) {
  return items.map(a => ({
    ...
    budget: a.budget ?? 0,
    spent: a.spent ?? 0,
    startDate: a.activity_date ?? a.startDate,
    endDate: a.endDate ?? a.activity_date,
    lead: a.lead ?? '—',
  }))
}
...
export default function Social() {
  ...
  const { data: rawCSR, loading: loadingCSR, isLive: csrLive, refetch: refetchCSR } = useApi(fetchCsrActivities, staticCSR)
  const csrActivities = csrLive ? normaliseCSR(rawCSR) : rawCSR
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Active CSR Programs', value: csrActivities.filter(a => a.status === 'Active').length, icon: Heart, color: 'bg-green-50 text-green-600' },
                ...
                <ResponsiveContainer width="60%" height="100%">
                  <PieChart>
                    <Pie data={diversityMetrics.gender} cx="50%" cy="50%" innerRadius={48} outerRadius={72} paddingAngle={3} dataKey="value">
                      {diversityMetrics.gender.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                    </Pie>
      ...
      {activeTab === 'CSR Activities' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <FilterBar activeFilter={statusFilter} onChange={setStatusFilter} filters={[
              { value: 'all', label: 'All' },
              { value: 'Active', label: 'Active' },
              { value: 'Completed', label: 'Completed' },
              { value: 'Upcoming', label: 'Upcoming' },
            ]} />
            <Button size="sm" variant="ghost" onClick={refetchCSR}><RefreshCw size={13} /></Button>
          </div>
          {loadingCSR ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({length:6}).map((_,i) => <Card key={i} className="h-48 animate-pulse bg-slate-50" />)}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {csrActivities
                .filter(a => (statusFilter === 'all' || a.status === statusFilter) && (a.title ?? '').toLowerCase().includes(search.toLowerCase()))
                .map(activity => (
          ...
          {csrLive && <p className="text-xs text-green-600">● Live data from backend</p>}
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {[
              ...
              <Card key={label}><p className="text-xl font-bold text-slate-900">{value}</p><p className="text-xs text-slate-500 mt-0.5">{label}</p></Card>
            ))}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card padding={false}>
            ...
            <Card padding={false}>
              <div className="px-5 pt-5 pb-3 border-b border-slate-200"><CardTitle>Ethnicity Distribution</CardTitle></div>
              <div className="p-5">
                    ...
                    return (
                      <div key={e.name}>
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-2">
                          <div className="h-2 rounded-full" style={{ width: `${e.value}%`, background: colors[i % colors.length] }} />
                        </div>
                      </div>
                    )
                  })}
          ...
          <div className="p-5" style={{ height: 300 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={engagementData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#64748B' }} axisLine={false} tickLine={false} />
                <YAxis yAxisId="left" domain={[60, 90]} tick={{ fontSize: 11, fill: '#64748B' }} axisLine={false} tickLine={false} />
      ...
      <Modal open={!!selectedCSR} onClose={() => setSelectedCSR(null)} title="CSR Activity Details" size="md">
        {selectedCSR && (
          <div className="space-y-3">
            ...
            {[['Title', selectedCSR.title], ['Lead', selectedCSR.lead], ['Participants', selectedCSR.participants], ['Impact', selectedCSR.impact], ['Budget', formatCurrency(selectedCSR.budget)], ['Start Date', selectedCSR.startDate ? formatDate(selectedCSR.startDate) : '—']].map(([k, v]) => (
